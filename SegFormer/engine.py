...
def train_step_CS(model: torch.nn.Module,
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        loss = loss_fn(y_preds, y.squeeze(dim=1).round().long())
        train_loss += loss.item()
        acc = torch.sum(y_preds_labels.type(torch.int32) == y.squeeze(dim=1).type(torch.int32))/y_preds_labels.numel()
        train_acc += acc.item()
        optimizer.zero_grad()
        loss.backward()
...
def test_step_CS(model: torch.nn.Module,
    ...
    with torch.inference_mode():
        for batch, (X, y) in enumerate(dataloader):
            ...
            loss = loss_fn(test_preds, y.squeeze(dim=1).round().long())
            acc = torch.sum(test_preds_labels.type(torch.int32) == y.squeeze(dim=1).type(torch.int32))/test_preds_labels.numel()
            test_loss += loss.item()
            test_acc += acc.item()
