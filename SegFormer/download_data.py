...
def download_data(dataset_name: str):
    ...
    if dataset_name == "ISBI":
        ...
        if any(item for item in data_path.iterdir()):
            ...
            print(f"Downloading dataset....")
            ade20k_data_path = kagglehub.dataset_download(handle="awsaf49/ade20k-dataset")
            print(f"Path to dataset files: {ade20k_data_path}")
            print(f"Copying data into working directory...")
            shutil.copytree(ade20k_data_path,
                            data_path,
                            dirs_exist_ok=True)
            print(f"Data copied to the working directory: {data_path}")
            shutil.rmtree(ade20k_data_path)
