...
discriminator_optim = torch.optim.RMSprop(discriminator.parameters(),
                                    lr= lr*0.1)
...
for epoch in tqdm(range(epochs)):
    ...
    for batch, (X, _) in tqdm(enumerate(train_dataloader)):
        ...
        _, disc_x_p = discriminator(x_p)
        discl_x_tilda, disc_x_tilda  = discriminator(x_tilda)
        discl_x, disc_x = discriminator(X)
    ...
    print(f" Epoch: {epoch} | Average Encoder Loss: {total_encoder_loss:.5f} | Average Decoder Loss: {total_decoder_loss:.5f} | Average Discriminator Loss: {total_discriminator_loss:.5f}")
